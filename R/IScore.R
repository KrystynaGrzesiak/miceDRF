...
Iscore_corr <- function(X, X_imp, multiple = TRUE, N = 50, imputation_func,
  ...
  scores_dat <- lapply(order(dim_with_NA, decreasing = TRUE), function(j) {
    ...
    if(!any(Oj)) {
      ...
      all_combinations <- unlist(lapply((1:ncol(M))[-j], function(i) {
        apply(t(combn((1:ncol(M))[-j], i - 1)), 1, paste0, collapse = ".")
      }))
      ...
      train_dat <- data.frame(combinations = all_combinations[-1],
                              nrow = unlist(lapply(all_combinations, function(ith) {
                                sum(grepl(ith, cand))}))[-1],
                              ncol = lengths(strsplit(all_combinations, ".", fixed = TRUE))[-1])
      ...
      observed_j_for_train <- !Oj_candidates[, cols_to_train[which.max(train_dat_single[["score"]])]] & !M[, j]
